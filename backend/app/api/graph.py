...
def build_graph():
    """
    Endpoint 2: Build graph by project_id
    ...
    try:
        ...
        if project.status == ProjectStatus.CREATED:
            return jsonify({
                "success": False,
                "error": "Project has not generated ontology yet, call /ontology/generate first"
            }), 400
        
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is building, do not resubmit. Add force: true for forced rebuild",
        ...
        logger.info(f"Creating graph building task: task_id={task_id}, project_id={project_id}")
        ...
        def build_task():
            ...
            try:
                ...
                # Wait for Zep processing (query each episode processed status)
                task_manager.update_task(
                    task_id,
...
def get_graph_data(graph_id: str):
    """
    Retrieve graph data (nodes and edges)
    """
    try:
        if not Config.ZEP_API_KEY:
            return jsonify({
