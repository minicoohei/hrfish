      ...
      <div class="status-group">
        ...
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: runStatus.twitter_completed }">
          <div class="platform-header">
            ...
            <span class="platform-name">Career Board</span>
          ...
          <div class="actions-tooltip">
            ...
            <div class="tooltip-actions">
              <span class="tooltip-action">EVALUATE</span>
              <span class="tooltip-action">ENDORSE</span>
              <span class="tooltip-action">SHARE</span>
              <span class="tooltip-action">CITE</span>
              <span class="tooltip-action">CONNECT</span>
        ...
        <div class="platform-status reddit" :class="{ active: runStatus.reddit_running, completed: runStatus.reddit_completed }">
          <div class="platform-header">
            ...
            <span class="platform-name">Career Discussion</span>
          ...
          <div class="actions-tooltip">
            <div class="tooltip-title">Available Actions</div>
            <div class="tooltip-actions">
              <span class="tooltip-action">ASSESS</span>
              <span class="tooltip-action">DISCUSS</span>
              <span class="tooltip-action">AGREE</span>
              <span class="tooltip-action">DISAGREE</span>
              <span class="tooltip-action">RESEARCH</span>
              <span class="tooltip-action">TREND</span>
              <span class="tooltip-action">CONNECT</span>
              <span class="tooltip-action">PASS</span>
              <span class="tooltip-action">REVIEW</span>
              <span class="tooltip-action">IDLE</span>
            </div>
          </div>
        </div>
...
const doStartSimulation = async () => {
  ...
  // Reset all state first to avoid impact from previous simulation
  resetAllState()
...
// Track previous round per platform to detect changes for logging
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.twitter_current_round > prevTwitterRound.value) {
        addLog(`[Board] R${data.twitter_current_round}/${data.total_rounds} | T:${data.twitter_simulated_hours || 0}h | A:${data.twitter_actions_count}`)
        prevTwitterRound.value = data.twitter_current_round
      }
      
      if (data.reddit_current_round > prevRedditRound.value) {
        addLog(`[Discussion] R${data.reddit_current_round}/${data.total_rounds} | T:${data.reddit_simulated_hours || 0}h | A:${data.reddit_actions_count}`)
        prevRedditRound.value = data.reddit_current_round
      }
      
      // Check if simulation completed (via runner_status or platform status)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra check: platform may report completion before runner_status updates
...
const checkPlatformsCompleted = (data) => {
  ...
  // Determine if platform is enabled via actions_count > 0 or was running
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // Return false if no platform enabled
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Get full action list via all_actions
...
const getActionTypeLabel = (type) => {
  const labels = {
    'CREATE_POST': 'EVALUATE',
    'REPOST': 'SHARE',
    'LIKE_POST': 'ENDORSE',
    'CREATE_COMMENT': 'DISCUSS',
...
const handleNextStep = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✗ Failed to start report: ${res.error || 'Unknown error'}`)
