<template>
  ...
  >
    <!-- Background decoration: grid lines (shown when projects exist) -->
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- Card title (first 20 chars of requirement) -->
        <h3 class="card-title">{{ getSimulationTitle(project.simulation_requirement) }}</h3>

        <!-- Card description (full requirement) -->
          ...
          <div class="modal-content">
            ...
            <div class="modal-actions">
              ...
              >
                ...
                <span class="btn-text">Resume Analysis</span>
              </button>
              <button 
              ...
              >
                ...
                <span class="btn-icon">◈</span>
                <span class="btn-text">Stakeholder Setup</span>
              </button>
              <button 
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 (Simulation) and Step5 (Interaction) must be launched while running and do not support historical replay</span>
            </div>
          </div>
        </div>
      </Transition>
...
let observer = null
let isAnimating = false  // Animation lock, prevent flickering
...
const containerStyle = computed(() => {
  ...
  // Calculate actual height: rows * card height + (rows-1) * gap + bottom padding
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate cards per row, ensure row centering
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Clear previous debounce timer (new scroll intent overrides)
        ...
        // If target equals current state, skip
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Debounce state transition to prevent rapid flickering
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay slightly before executing, avoid too fast transition
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
onMounted(async () => {
  ...
  // Wait for DOM rendering to initialize observer
  setTimeout(() => {
    initObserver()
  }, 100)
})
...
.grid-pattern {
  ...
  /* Position from top-left, height changes only extend at bottom */
...
.status-icon:nth-child(1).available { color: #3B82F6; } /* Resume Analysis - blue */
.status-icon:nth-child(2).available { color: #F59E0B; } /* Stakeholder Setup - orange */
