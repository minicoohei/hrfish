    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content still processing. Try manually refreshing the graph later.</span>
...
const wasSimulating = ref(false) // Track if simulation was previously running
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transition from simulating to not simulating, show finished hint
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loop edge - add only one merged entry per node
    ...
    // Calculate curvature: spread out for multiple edges, straight for single
    let curvature = 0
    if (totalCount > 1) {
      ...
      // Flip curvature if edge direction is reversed from canonical
      // This ensures all edges distribute in the same frame, preventing overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add center gravity to cluster isolated nodes toward center
  ...
  const getLinkPath = (d) => {
    ...
    // More edges = larger proportion of offset
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Base 25%, +5% per edge
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Record position only, don't restart simulation (distinguish click from drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect actual drag start (moved beyond threshold)
      ...
      .on('end', (event, d) => {
        // Only decelerate simulation if actually dragged
        if (d._isDragging) {
