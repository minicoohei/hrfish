...
# Adding this prefix prevents Agent from calling tools, forcing text reply
...
def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize interview prompt by adding prefix to prevent tool calls
...
def get_graph_entities(graph_id: str):
    ...
    Only return nodes matching predefined entity types (labels beyond just Entity)
    
    Query params:
        entity_types: Comma-separated entity types (optional, for further filtering)
...
def create_simulation():
    ...
    Note: max_rounds and other params are LLM-generated, no manual setup needed
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: run scripts (run_*.py) remain in backend/scripts/, no longer copied to simulation dir
...
def prepare_simulation():
    """
    Prepare simulation environment (async task, LLM intelligent param generation)
    ...
    4. LLM intelligent simulation config generation (with retry)
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Preparation task started|Completed preparation available",
                "already_prepared": true|false    // preparation status
            }
        }
    """
    import threading
    ...
    try:
        ...
        if not force_regenerate:
            logger.debug(f"Check simulation {simulation_id} whether already prepared...")
        ...
        # ========== Synchronously getting entity count (before background task starts) ==========
        # So frontend can get expected agent count immediately after calling prepare
        try:
            logger.info(f"Synced retrieved entity count: graph_id={state.graph_id}")
            reader = ZepEntityReader()
            ...
            logger.warning(f"Failed to get entity count synchronously (will retry in background): {e}")
...
def get_prepare_status():
    ...
    try:
        ...
        # If simulation_id provided, check if preparation is complete
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Completed preparation available",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
        ...
        if not task:
            # Task does not exist, but if simulation_id provided, check if preparation is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def get_simulation(simulation_id: str):
    ...
    try:
        ...
        # If simulation is prepared, attach run instructions
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    """
    Get latest report_id for a simulation
    
    Traverse reports directory to find report matching simulation_id,
    return the latest one (sorted by created_at) if multiple exist
...
def get_simulation_history():
    """
    Retrieve historical simulation list (with project details)
    
    For homepage history display, returns simulation list with project name, description, and rich info
    
    Query params:
        limit: Result count limit（default20）
    ...
    try:
        ...
        logger.error(f"Failed to retrieve historical simulations: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Realtime get simulation agent profiles (for viewing progress during generation)
    ...
    try:
        ...
        logger.error(f"Failed to retrieve profiles in realtime: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to retrieve config in realtime: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        - time_config: Time config (simulation duration, rounds, peak/off-peak hours)
        - agent_configs: Each agent activity config (activity level, posting frequency, stance, etc.)
        - event_config: Event config (initial posts, trending topics)
...
def start_simulation():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",          // Required,Simulation ID
            "platform": "parallel",                // Available: twitter / reddit / parallel (default)
            "max_rounds": 100,                     // optional: max simulation rounds for truncating long simulations
            "enable_graph_memory_update": false,   // Available: Whether to dynamically update agent activities to Zep graph memory
            "force": false                         // optional: force restart (stops running simulation and clears logs)
        }

    About the force parameter:
        - When enabled, stops running/completed simulation and clears run logs
        ...
        - For scenarios requiring simulation re-run
    ...
    About enable_graph_memory_update:
        ...
        - Allows graph to "remember" simulation process for subsequent analysis or AI chat
        - Requires associated project to have a valid graph_id
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                return jsonify({
                    "success": False,
                    "error": f"Simulation not prepared, current status: {state.status.value}, call /prepare first"
                }), 400
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "Graph memory update requires valid graph_id, ensure project has built graph"
                }), 400
...
def get_run_status(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to retrieve run status: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_run_status_detail(simulation_id: str):
    """
    Retrieve simulation detailed run status (including all actions)
    ...
    try:
        ...
        # Retrieve basic status info
        result = run_state.to_dict()
        result["all_actions"] = [a.to_dict() for a in all_actions]
        result["twitter_actions"] = [a.to_dict() for a in twitter_actions]
        result["reddit_actions"] = [a.to_dict() for a in reddit_actions]
        result["rounds_count"] = len(run_state.rounds)
        # recent_actions shows current latest round content for both platforms
        ...
        logger.error(f"Failed to retrieve detailed status: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_timeline(simulation_id: str):
    """
    Retrieve simulation timeline (round summary)
...
def get_agent_stats(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to retrieve agent statistics: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (entered command wait mode after simulation loop)
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // Optional,Specify platform（twitter/reddit）
                                               // if not specified: dual-platform sim interviews both platforms
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment not running or closed. Ensure simulation is complete and in command wait mode."
            }), 400
        ...
        return jsonify({
            "success": False,
            "error": f"Waiting for interview response timeout: {str(e)}"
        }), 504
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // Required,Interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What is your view on B?"  // uses default if platform not specified
                }
            ],
            "platform": "reddit",              // optional, default platform (overridden by each item's platform)
                                               // if not specified: dual-platform sim interviews each agent on both platforms
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "error": f"Waiting for batch interview response timeout: {str(e)}"
        }), 504
...
def interview_all_agents():
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "error": f"Waiting for global interview response timeout: {str(e)}"
        }), 504
    ...
    Request (JSON):
        {
            ...
            "limit": 100                   // Optional,Result count,default100
        }
...
def get_env_status():
    """
    Retrieve simulationEnvironment status
...
def close_simulation_env():
    ...
    Note: This differs from /stop endpoint, which forcefully terminates the process,
    while this endpoint gracefully shuts down the simulation environment.
