...
class SimulationRunState:
    """Simulation run status (realtime)"""
    simulation_id: str
...
class SimulationRunner:
    ...
    2. Parse run logs, record each agent action
    ...
    def start_simulation(
        ...
        graph_id: str = None  # Zep graph ID (required when graph update enabled)
    ) -> SimulationRunState:
        """
        Start simulation
        ...
        Args:
            ...
            graph_id: Zep graph ID (required when graph update enabled)
        ...
        try:
            ...
            # If max rounds specified, add to command args
            if max_rounds is not None and max_rounds > 0:
            ...
            # Set working directory to simulation dir (database files generated here)
            # Use start_new_session=True to create new process group, ensuring os.killpg can terminate all subprocesses
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group, ensure all related processes terminate on server shutdown
            )
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Update round info (from round_end event)
                                    ...
                                    # Total round is max of both platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Total time is max of both platforms
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        """
        Cross-platform process and subprocess termination
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer record platform, otherwise use default
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean simulation run logs (for forcing simulation restart)
        ...
        # Directories to delete (containing action logs)
        ...
        logger.info(f"Simulation log cleanup complete: {simulation_id}, Delete files: {cleaned_files}")
    ...
    def cleanup_all_simulations(cls):
        ...
        Called on server shutdown to ensure all subprocesses are terminated
        """
        ...
        # Check if there is anything to clean up (avoid useless log printing)
    ...
    def register_cleanup(cls):
        ...
        # In non-debug mode, this env var is absent, still need to register
    ...
    def get_running_simulations(cls) -> List[str]:
        """
        Retrieve list of all running simulation IDs
        """
        running = []
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Retrieve simulation environment detailed status info
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: In dual-platform simulation, interview both platforms simultaneously and return integrated results
            timeout: Timeout (seconds)

        Returns:
            Interview results dictionary

        Raises:
            ValueError: Simulation does not exist or environment not running
            TimeoutError: Waiting for response timeout
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, can be overridden per interview item)
                - "twitter": defaultInterview Twitter platform only
                - "reddit": defaultInterview Reddit platform only
    ...
    def interview_all_agents(
        ...
        # Retrieve all agent info from config file
        config_path = os.path.join(sim_dir, "simulation_config.json")
        if not os.path.exists(config_path):
    ...
    def _get_interview_history_from_db(
        ...
        """Retrieve interview history from single database"""
        import sqlite3
    ...
    def get_interview_history(
        ...
        """
        Retrieve interview history (from database)
        ...
        Args:
            ...
            platform: Platform type（reddit/twitter/None）
                ...
                - None: Retrieve all history from both platforms
            agent_id: Specified agent ID (optional, only retrieve this agent's history)
