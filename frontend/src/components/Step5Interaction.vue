    ...
    <div class="main-split-layout">
      ...
      <div class="left-panel report-style" ref="leftPanel">
        <div v-if="reportOutline" class="report-content-wrapper">
          ...
          <div class="report-header-block">
            <div class="report-meta">
              <span class="report-tag">キャリア分析レポート</span>
        ...
        <div v-if="!reportOutline" class="waiting-placeholder">
          ...
          <span class="waiting-text">レポートエージェントを待機中...</span>
        </div>
      </div>
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="action-bar-header">
          ...
          <div class="action-bar-text">
            <span class="action-bar-title">インタラクティブツール</span>
            <span class="action-bar-subtitle mono">{{ profiles.length }} 名のエージェント</span>
          </div>
        </div>
          <div class="action-bar-tabs">
            ...
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.username : 'エージェントを選択して会話' }}</span>
                <svg class="dropdown-arrow" :class="{ open: showAgentDropdown }" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2">
                  <polyline points="6 9 12 15 18 9"></polyline>
                </svg>
              </button>
              <div v-if="showAgentDropdown" class="dropdown-menu">
                <div class="dropdown-header">会話相手を選択</div>
                <div 
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                <div class="tools-card-name">レポートエージェント - チャット</div>
                <div class="tools-card-subtitle">4つの専門ツールとシミュレーション記憶を備えたキャリア分析エージェント</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">InsightForge 深層帰属分析</div>
                    <div class="tool-desc">実データとシミュレーション状態を照合し、グローバル/ローカルメモリによる時系列横断の深層帰属分析を実行</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">PanoramaSearch 全追跡</div>
                    <div class="tool-desc">グラフベースの幅優先探索で評価伝播経路を再構築し、情報フローのトポロジーを把握</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">GraphRAGベースの即時クエリ。最適化インデックスでノード属性と離散的ファクトを高速抽出</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">シミュレーションエージェントとの並列多ラウンド対話を自動実行し、非構造化意見データと心理状態を収集</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? 'レポートエージェントと会話してレポートの内容を深掘りできます' : 'シミュレーションエージェントと会話して意見を聞けます' }}
              </p>
            </div>
            <div 
              ...
              <div class="message-content">
                <div class="message-header">
                  <span class="sender-name">
                    {{ msg.role === 'user' ? 'あなた' : (chatTarget === 'report_agent' ? 'レポートエージェント' : (selectedAgent?.username || 'エージェント')) }}
                  </span>
        ...
        <div v-if="activeTab === 'survey'" class="survey-container">
          ...
          <div class="survey-setup">
            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">アンケート対象を選択</span>
                <span class="selection-count">{{ selectedAgents.size }} / {{ profiles.length }} 名選択中</span>
              </div>
              ...
              <div class="selection-actions">
                <button class="action-link" @click="selectAllAgents">全選択</button>
            ...
            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">アンケート質問</span>
              </div>
              <textarea
                v-model="surveyQuestion"
                class="survey-input"
                placeholder="選択した対象全員に聞きたい質問を入力してください..."
                rows="3"
              ></textarea>
            </div>
          ...
          <div v-if="surveyResults.length > 0" class="survey-results">
            <div class="results-header">
              <span class="results-title">アンケート結果</span>
...
const renderMarkdown = (content) => {
  ...
  // Clean <p><br> immediately before block elements (extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean <br> between block elements and paragraph start tags
