...
Run Twitter and Reddit simulations simultaneously, reading the same config files

Features:
- Dual-platform (Twitter + Reddit) parallel simulation
- After simulation, enters command wait mode instead of closing immediately
...
# Fixes OASIS third-party library reading files without specifying encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Affects all open() calls without explicit encoding
    os.environ.setdefault('PYTHONUTF8', '1')
    os.environ.setdefault('PYTHONIOENCODING', 'utf-8')
    
    # Reconfigure stdout streams to UTF-8 (fix console encoding issues)
    ...
    # Note: must be set at Python startup, runtime setting may not take effect
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        Fixes third-party libraries (e.g., OASIS) reading files without specifying encoding
        """
...
# Script located in backend/scripts/ directory
...
# Add filter immediately on module load to ensure it takes effect before camel code executes
...
def disable_oasis_logging():
    ...
    OASIS logging is too verbose (logs every agent observation/action), we use our own action_logger
    """
    ...
    for logger_name in oasis_loggers:
        ...
        logger.setLevel(logging.CRITICAL)  # Log only critical errors
...
def init_logging_for_simulation(simulation_dir: str):
    """
    Initialize simulation log config
...
class ParallelIPCHandler:
    ...
    Manage dual-platform environments, handle Interview commands
    """
    ...
    def poll_command(self) -> Optional[Dict[str, Any]]:
        ...
        # Retrieve command files (sorted by time)
        command_files = []
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        ...
        Returns:
            Dict containing result, or dict containing error
        """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None/not specified: Interview both platforms simultaneously, return integrated results
            
        Returns:
            True indicates success,False indicates failure
        """
        # If platform specified, interview that platform only
        ...
        if success_count > 0:
            ...
            print(f"  InterviewComplete: agent_id={agent_id}, successful platforms={success_count}/{len(platforms_to_interview)}")
            return True
        else:
            ...
            try:
                ...
                for interview in twitter_interviews:
                    ...
                    try:
                        ...
                        print(f"  Warning: cannot retrieve Twitter Agent {agent_id}: {e}")
            ...
            try:
                ...
                for interview in reddit_interviews:
                    ...
                    try:
                        ...
                        print(f"  Warning: cannot retrieve Reddit Agent {agent_id}: {e}")
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    """
    Retrieve agent_id -> entity_name mapping from simulation_config
...
def fetch_new_actions_from_db(
    ...
    """
    Retrieve new action records from database and supplement with full context info
    
    Args:
        db_path: Database file path
        last_rowid: Last read max rowid value (using rowid instead of created_at because created_at format differs across platforms)
    ...
    try:
        ...
        # Use rowid to track processed records (rowid is SQLite built-in auto-increment field)
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Slim down action_args, keep only key fields (preserve full content, no truncation)
            simplified_args = {}
            ...
            # Supplement context info (post content, usernames, etc.)
...
def _enrich_action_context(
    ...
    """
    Supplement action context info (post content, usernames, etc.)
    ...
    try:
        ...
        # Quote post: supplement original post content, author, and quote comment
            ...
            # Retrieve quote post comment content (quote_content)
            if new_post_id:
                cursor.execute("""
        ...
        # Follow user: supplement followed user name
            ...
            if follow_id:
                # Retrieve followee_id from follow table
                cursor.execute("""
        ...
        # Mute user: supplement muted user name
            ...
            # Retrieve user_id or target_id from action_args
        ...
        # Post comment: supplement commented post info
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports dual LLM config for parallel simulation speedup:
    ...
    If boost LLM is configured, parallel simulation can use different API providers per platform for higher concurrency.
    ...
    # Select which LLM to use based on params and config
...
async def run_twitter_simulation(
    ...
    # Retrieve agent real name mapping from config (using entity_name instead of default Agent_X)
    ...
    for round_num in range(total_rounds):
        ...
        # Retrieve actually executed actions from database and log them
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM config (if available, falls back to general config)
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close environment immediately after simulation complete, do not enter command wait mode'
    )
    
    args = parser.parse_args()
    
    # Create shutdown event at main function start, ensure entire program can respond to exit signals
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: do not exit after simulation complete, wait for interview commands
    When termination signal is received:
    1. Notify asyncio loop to exit waiting
    2. Allow program to properly clean up resources (close database, environment, etc.)
    3. Then exit
    """
